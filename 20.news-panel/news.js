window.onload = function(){
    ...
    var lastTabIndex = 0; //记录上一次tab的位置

    var lastNewsIndex = 0; //记录上一次news的位置
    ...
    //新闻Panel的鼠标移入(停止新闻自动播放)、移出(开启新闻自动播放)事件
    ...
    function fillTabs(arr){
        ...
        for(var i = 0; i < arr.length; i++){
            ...
            tab[i].style.left = panelW /arr.length * i + 'px'; //根据数量,计算每一个tab的左偏移量
        }
    ...
    function fillNews(name){
        ...
        for ( var i = 0; i < news.length; i++){
            if(news[i].column == name){
                ...
                if(columnArr.length == 3) break; //最多显示三条新闻
            }
        }
    ...
    function bindNewsEvent(){
        for(var i = 0; i < items.length; i++){
            ...
            (function(index){
                items[index].onmouseover = function(ev){
                    clearInterval(newslist.timer);
                    var ev = ev||event;
                    highlightNews(index);
                    ev.stopPropagation();
    ...
    function highlightTab(num){
        ...
        tab[num].className = 'tab active';
    }


    //高亮当前news
    function highlightNews(num) {
        for(var i = 0; i < items.length; i++){
